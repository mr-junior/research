...
int main(int argc, char* argv[])
{
  ...
  pen.setWidth(0);
  QBrush brush(Qt::red);
  QGraphicsScene* scene = new QGraphicsScene(0, 0, matrix.size()*r+1, matrix.size()*r+1);
  ...
  QImage image(scene->sceneRect().size().toSize(), QImage::Format_RGB444); // Create the image with the exact size of the shrunk scene
  image.fill(Qt::white); // Start all pixels white

  QPainter painter(&image);
  scene->render(&painter);
  image.save(argv[2], Q_NULLPTR, 0);
  return 0;
}
