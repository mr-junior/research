...
namespace converter {
      ...
      , const std::string& s_g_f_n)
  {
    ...
    for(boost::tie(e, e_end) = boost::edges(graph); e_end != e; ++e)
    {
      ...
      for (boost::tie(next, end) = vertices(graph); next != end; ++next, ++i)
      {
        if (s == *next)
        {
          s_index = i;
  ...
  namespace {
        ...
        , const int N, undirected_graph& g)
    {
      ...
      std::string trimmed_line = boost::trim_copy_if(line, boost::is_any_of(" \t\r"));
      boost::split(splitted, trimmed_line, boost::is_any_of(" \t\r"), boost::token_compress_on);
      if(2 != splitted.size())
      {
        throw exception(wrong_format_of_standart_graph_file);
      }
      int u = boost::lexical_cast<int>(splitted[0]);
      int v = boost::lexical_cast<int>(splitted[1]);
      if(u >= N || v >= N)
      {
        throw exception(wrong_format_of_standart_graph_file);
      }
